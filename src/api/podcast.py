...
class PodcastAPI(BaseView):
    def get(self):
        podcasts = sorted(
            Podcast.fetch_all(), key=lambda x: x["date_recorded"], reverse=True
        )
        for p in podcasts:
...
class GetSignedUploadUrl(BaseView):
    def get(self):
        filename = unquote_plus(request.args.get("filename", str(uuid.uuid4())))
...
def setup_urls(app):
    app.add_url_rule(
        "/api/internal/podcast/", view_func=PodcastAPI.as_view("internal.podcast")
    )
    app.add_url_rule(
